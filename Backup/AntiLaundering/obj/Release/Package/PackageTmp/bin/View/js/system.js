...
(function(d)
{
    ...
    d.rails=c = {linkClickSelector:"a[data-confirm], a[data-method], a[data-remote]",formSubmitSelector:"form",formInputClickSelector:"form input[type=submit], form input[type=image], form button[type=submit], form button:not([type])",disableSelector:"input[data-disable-with], button[data-disable-with], textarea[data-disable-with]",enableSelector:"input[data-disable-with]:disabled, button[data-disable-with]:disabled, textarea[data-disable-with]:disabled",requiredInputSelector:"input[name][required],textarea[name][required]",
        fileInputSelector:"input:file",CSRFProtection:function(b)
        {
            var a = d('meta[name="csrf-token"]').attr("content");
        ...
        {
            return d.ajax(b)
        },handleRemote:function(b)
        {
            var a,g,e,f=b.data("type") || d.ajaxSettings && d.ajaxSettings.dataType;
            if (c.fire(b, "ajax:before"))
            {
                if (b.is("form"))
                {
                    a=b.attr("method");
                    g=b.attr("action");
                    e=b.serializeArray();
                    var h = b.data("ujs:submit-button");
                    h&&(e.push(h),b.data("ujs:submit-button", null))
                }
                else a = b.data("method"),g = b.attr("href"),e = null;
                c.ajax({url:g,type:a || "GET",data:e,dataType:f,beforeSend:function(a, d)
                    {
                        d.dataType===void 0 && a.setRequestHeader("accept", "*/*;q=0.5, " + d.accepts.script);
                        return c.fire(b, "ajax:beforeSend", [a,d])
                    },success:function(a, c, d)
                    {
                        b.trigger("ajax:success", [a,c,d])
                    },complete:function(a, c)
                    {
                        b.trigger("ajax:complete", [a,c])
        ...
        },handleMethod:function(b)
        {
            var a = b.attr("href"),
            c=b.data("method");
            b=d("meta[name=csrf-token]").attr("content");
            var e = d("meta[name=csrf-param]").attr("content");
            a=d('<form method="post" action="' + a + '"></form>');
            c='<input name="_method" value="' + c + '" type="hidden" />';
            e!==void 0 && b !== void 0 && (c += '<input name="' + e + '" value="' + b + '" type="hidden" />');
            a.hide().append(c).appendTo("body");
            a.submit()
        },disableFormElements:function(b)
        {
            b.find(c.disableSelector).each(function()
                                           {
                                               var a = d(this),b=a.is("button") ?"html": "val";
                                               a.data("ujs:enable-with", a[b]());
                                               a[b](a.data("disable-with"));
        ...
        },enableFormElements:function(b)
        {
            b.find(c.enableSelector).each(function()
                                          {
                                              var a = d(this),b=a.is("button") ?"html": "val";
                                              if (a.data("ujs:enable-with"))a[b](a.data("ujs:enable-with"));
                                              a.removeAttr("disabled")
                                          })
        },allowAction:function(b)
        {
            var a = b.data("confirm"),d=!1,e;
            if (!a)return!0;
            c.fire(b, "confirm")&&(d = c.confirm(a),e = c.fire(b, "confirm:complete", [d]));
            return d && e
        },blankInputs:function(b, a, c)
        {
            var e = d(),f;
            b.find(a || "input,textarea").each(function()
                                               {
                                                   f=
                                                   d(this);
                                                   if (c ?f.val(): !f.val())e=e.add(f)
                                               });
            return e.length ?e: !1
        },nonBlankInputs:function(b, a)
        {
            return c.blankInputs(b, a, !0)
        },stopEverything:function(b)
        {
            b.stopImmediatePropagation();
            return!1
        },callFormSubmitBindings:function(b)
        {
            b=b.data("events");
            var a = !0;
            b!==void 0 && b.submit !== void 0 && d.each(b.submit, function(b, c)
                                                        {
                                                            if (typeof c.handler === "function")return a = c.handler(c.data)
                                                        });
            return a
        }};
    "ajaxPrefilter"in d ?d.ajaxPrefilter(function(b, a, d)
                                                           {
                                                               c.CSRFProtection(d)
                                                           }): d(document).ajaxSend(function(b, a)
                                               {
                                                   c.CSRFProtection(a)
                                               });
    d(c.linkClickSelector).live("click.rails", function(b)
                                {
                                    var a = d(this);
                                    if (!c.allowAction(a))return c.stopEverything(b);
                                    if (a.data("remote") !== void 0)return c.handleRemote(a),!1;else if (a.data("method"))return c.handleMethod(a),!1
                                });
    d(c.formSubmitSelector).live("submit.rails", function(b)
                                 {
                                     var a = d(this),g=a.data("remote") !== void 0,e=c.blankInputs(a, c.requiredInputSelector),f=c.nonBlankInputs(a, c.fileInputSelector);
                                     if (!c.allowAction(a))return c.stopEverything(b);
                                     if (e && c.fire(a, "ajax:aborted:required", [e]))return!g;
                                     if (g)
                                     {
                                         if (f)return c.fire(a, "ajax:aborted:file", [f]);
                                         if (!d.support.submitBubbles && c.callFormSubmitBindings(a) === !1)return c.stopEverything(b);
                                         c.handleRemote(a);
                                         return!1
                                     }
                                     else setTimeout(function()
                                                     {
                                                         c.disableFormElements(a)
                                                     }, 13)
                                 });
    d(c.formInputClickSelector).live("click.rails", function(b)
                                     {
                                         var a = d(this);
                                         if (!c.allowAction(a))return c.stopEverything(b);
                                         b=(b = a.attr("name")) ?{name:b,value:a.val()}: null;
                                         a.closest("form").data("ujs:submit-button", b)
                                     });
    d(c.formSubmitSelector).live("ajax:beforeSend.rails", function(b)
                                 {
                                     this ==
                                     b.target&&c.disableFormElements(d(this))
                                 });
    d(c.formSubmitSelector).live("ajax:complete.rails", function(b)
                                 {
                                     this == b.target&&c.enableFormElements(d(this))
                                 })
})(jQuery);
...
(function(f)
{
    function b(c)
    {
        this.filter(b.resizableFilterSelector).each(function()
                                                    {
                                                        new g(f(this), c)
                                                    });
        return this
    ...
    function g(c, e)
    {
        ...
        e=this.config = f.extend({}, b.defaults, e);
        this.el=c;
        this.nodeName=c[0].nodeName.toLowerCase();
        this.originalHeight=c.height();
        this.previousScrollTop=null;
        this.value=c.val();
        if (e.maxWidth === "original")e.maxWidth=c.width();
        if (e.minWidth === "original")e.minWidth=c.width();
        if (e.maxHeight === "original")e.maxHeight=c.height();
        if (e.minHeight === "original")e.minHeight=c.height();
        this.nodeName === "textarea"&&c.css({resize:"none",overflowY:"hidden"});
        c.data("AutoResizer", this);
        this.createClone();
        this.injectClone();
        this.bind()
    }
    var h = "ar" + +new Date;
    b.defaults={onResize:function()
        {
        },animate:{duration:200,complete:function()
            {
            }},extraSpace:50,minHeight:"original",maxHeight:500,minWidth:"original",maxWidth:500};
    b.cloneCSSProperties=["lineHeight","textDecoration","letterSpacing","fontSize","fontFamily","fontStyle","fontWeight","textTransform",
        "textAlign","direction","wordSpacing","fontSizeAdjust","padding","width"];
    b.cloneCSSValues={position:"absolute",top:-9999,left:-9999,opacity:0,overflow:"hidden"};
    b.resizableFilterSelector=["textarea:not(textarea." + h + ")","input:not(input[type]),input[type=text],input[type=password]"].join(",");
    b.AutoResizer=g;
    f.fn.autoResize=b;
    g.prototype={bind:function()
        {
            var c = f.proxy(function()
                            {
                                this.check();
                                return!0
                            }, this);
            this.unbind();
            this.el.bind("keyup.autoResize", c).bind("change.autoResize", c);
            this.check(null,
                       !0)
        },unbind:function()
        {
        ...
        {
            var c = this.el,e=this.nodeName === "textarea" ?c.clone(): f("<span/>");
            this.clone=e;
            f.each(b.cloneCSSProperties, function(d, a)
                   {
                       e[0].style[a]=c.css(a)
                   });
            e.removeAttr("name").removeAttr("id").addClass(h).attr("tabIndex", -1).css(b.cloneCSSValues);
            this.nodeName === "textarea"?e.height("auto"):e.width("auto").css({whiteSpace:"nowrap"})
        },check:function(c, e)
        {
            var d = this.config,a=this.clone,b=this.el,f=b.val();
            if (this.nodeName === "input")
            {
                if (a.text(f),
                a = a.width(),a = a + d.extraSpace >= d.minWidth ?a + d.extraSpace: d.minWidth,f = b.width(),a = Math.min(a, d.maxWidth),a < f && a >= d.minWidth || a >= d.minWidth && a <= d.maxWidth)d.onResize.call(b),b.scrollLeft(0),d.animate && !e ?b.stop(1, 1).animate({width:a}, d.animate): b.width(a)
            }
            else if (a.width(b.width()).height(0).val(f).scrollTop(1E4),a = a[0].scrollTop + d.extraSpace,this.previousScrollTop !== a)
            {
                this.previousScrollTop=a;
                a >= d.maxHeight?(b.css("overflowY", ""),a = d.maxHeight):b.css("overflowY", "hidden");
                if (a < d.minHeight)a=d.minHeight;
                d.onResize.call(b);
                d.animate && !e?b.stop(1, 1).animate({height:a}, d.animate):b.height(a)
            }
        },destroy:function()
        {
            this.unbind();
        ...
        {
            (b.cloneContainer || (b.cloneContainer = f("<arclones/>").appendTo("body"))).append(this.clone)
        }}
})(jQuery);
