...
namespace AntiLaundering.View.UserManagement
{
    ...
    public partial class AddSqlUser : System.Web.UI.Page
    {
        ...
        protected void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            //}
            MyCustomMembershipUser user = smp.CreateUser(comboEmployee.Text, txtUsername.Text, txtPassword.Text, this.txtEmail.Text.Trim(), txtComment.Text, txtPhone.Text, approved, null, empid, out status);

            if (user != null)
            {
                smp.UpdateUser(smp.GetUser(txtUsername.Text, false));
                foreach (ListItem rolebox in cblUserRoles.Items)
                {
                    if (rolebox.Selected)
                    {
